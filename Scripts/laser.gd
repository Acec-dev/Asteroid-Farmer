# laser.gd - Continuous beam weapon with ramping damage
extends Node2D

@export var max_range: float = 1200.0
@export var base_damage: float = 2.0  # Damage per tick at minimum
@export var max_damage: float = 50.0  # Maximum damage per tick
...
func _apply_damage(target: Node) -> void:
	...
	var current_damage = lerp(base_damage, max_damage, ramp_progress)
	
	# Apply damage (convert to hits - asteroids expect integer hits)
	# We'll apply fractional damage by tracking and applying full hits
	if current_damage >= 1.0:
		var hits_to_apply = int(current_damage)
		for i in hits_to_apply:
			target.hit_by_projectile(self)
